...
document.form1.suppliername.value = document.form1.district.value;
}
...
<div align="center"><br>
  <h2 align="center"><span style="color:#F17327;">Debit Note creation form</span>    </h2>
  <form id="form1" name="form1" method="post" onSubmit="return ValidateForm()" action="a9debitnote_back.php">
  <table border="1" rules="none" frame="box" cellpadding="1">
      <tbody>
        <tr>
          <td colspan="4"  bgcolor="#14C4B6"><h4>Select Supplier Details</h4>
          </td>
        </tr>
		 <tr>
          <td>Transaction Date:
          </td><td><input id="saudadate"  onchange = "isDate(this.value)"  name = "tdate"  autofocus  type = "text"  size="17" required="required" />
          <a href="javascript:NewCal('saudadate','ddmmyyyy')"><img src="../img/cal.gif" width="16" height="16" border="0" alt="Pick a date"></a></td>
          <td>Credit Ledger</td>
          <td>
          <select name="credit"   onChange="hledger(this)"  style="width:170px">
				...
				while($row = mysql_fetch_array($query)){
                  ...
                  echo"<option  value = $exid>$expenseshead</option>"; 
                     ...
                     </select>  <input type="text" name="ledgername_n" id="ledgername_n"  />
          </td>
        </tr>
        <tr>
	...
	while($query_data = mysql_fetch_array($result_dispMake)){
	 ...
	 ?> 
        ...
        <tr>
          ...
          <td>Bill Value:</td>
          <td>
          <input type="text" required = "required"  name="billvalue"/></td>
        </tr>
        <tr>
        ...
        <tr>
          <td>Party Name</td>
          <td colspan="2"><input type="text"  readonly name="suppliername" size="40"/></td>
