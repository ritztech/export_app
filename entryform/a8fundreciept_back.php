...
$qry="INSERT INTO mandia8 (supid,siid,suppliername,type,amount,bankcharges,total,entrydate,fid,saudadate)
 VALUES ('$province','$district','$row111[0]','$type','$tamount','$tbcharges','$tbtotal',STR_TO_DATE('$entrydate','%d/%m/%Y'),'$fid',STR_TO_DATE('$saudadate','%d/%m/%Y'))";
 ...
 $result1 = mysql_query("SELECT max(a8id) from mandia8");
...
VALUES ('$row11[0]','$row1[0]','{$_POST['bbranch'.$i]}','{$_POST['chkno'.$i]}','{$_POST['amount'.$i]}','{$_POST['bcharges'.$i]}','{$_POST['btotal'.$i]}','{$_POST['modepay'.$i]}','{$_POST['bremark'.$i]}','A8','{$_POST['bname'.$i]}')";
  ...
  {
	  echo "<script>alert('Data Successfully Inserted');location.href='a8fundreciept_front.php'</script>";
