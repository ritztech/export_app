...
if(isset($_POST['s'])) {
		   ...
		   $saudadate=$_POST['saudadate'];
			...
			$newsiid = trim(strip_tags(addslashes($_POST['newsiid'])));
 ...
 //  sudhir
  ...
  die('Error: ' . mysql_error());
	...
	$qry1 = "DELETE FROM mandia7_ref WHERE tempid  = 'A8' and formid = $a8id ";
...
VALUES ('{$_POST['bname'.$i]}',$a8id,'{$_POST['bbranch'.$i]}','{$_POST['chkno'.$i]}','{$_POST['amount'.$i]}','{$_POST['bcharges'.$i]}','{$_POST['btotal'.$i]}','{$_POST['modepay'.$i]}','{$_POST['bremark'.$i]}','A8','{$_POST['ledger'.$i]}')";
  ...
  die('Error: ' . mysql_error());
	 ...
	 if ($j > $k)
      ...
      $row11 = mysql_fetch_array($result11);
	    ...
	    $qry2="INSERT INTO mandia7_ref(bankname,formid,branch,chequeno,amount,bankcharges,total,modeofpay,remark,tempid,ledger_id)
VALUES ('$row11[0]',$a8id,'{$_POST['bbranch'.$i]}','{$_POST['chkno'.$i]}','{$_POST['amount'.$i]}','{$_POST['bcharges'.$i]}','{$_POST['btotal'.$i]}','{$_POST['modepay'.$i]}','{$_POST['bremark'.$i]}','A8','{$_POST['bname'.$i]}')";
  ...
  if ($j = $k)
       { 
	    ...
	    echo "<script>alert('Data updated successfully.');location.href='a8fundreciept_edit.php?a8id=$a8id'</script>"; 
...
try {
$query = "SELECT `a8id`, `fid`, `siid`, `supid`, `suppliername`, `type`, `entrydate`,DATE_FORMAT(saudadate,'%d/%m/%Y') as saudadate, `amount`, `bankcharges`, `total` FROM `mandia8` WHERE a8id=?";
$stmt = $dbc->prepare($query);
$stmt->bindParam(1, $_GET['a8id']);
$stmt->execute();
...
<div align="center"><br>
  <h2 align="center"><span style="color:#F17327;"> Edit Fund Payment Details</span>    </h2>
  ...
  <table width="749" border="1" rules="none" frame="box" cellpadding="6">
      <tbody>
        ...
        <tr>
          ...
          <td colspan="2"><select  onChange="getDistrict(this.value),h456(this,this.value)"  name="province" id="province" style="150px">
	 ...
	 ?> 
        ...
        <tr>
          ...
          <td><label for="textfield">Payment Type:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
           ...
           <input  type = "hidden"  size = "5" name = "newsiid"  value="<?php echo $newsiid?>" /> 
        </tr>
        <tr>
...
<tr>
  ...
  <?php 
$result13 = mysql_query("SELECT `bankname`, `formid`, `branch`, `chequeno`, `amount`, `bankcharges`, `total`, `modeofpay`, `remark`, `tempid`,ledger_id FROM `mandia7_ref` WHERE formid=$a8id
and tempid='A8'");
?>
  <?php
...
<input  type = "hidden"  size = "5" name = "a8id"  value="<?php echo $a8id?>" /> 
&nbsp;</td>
        </tr>
