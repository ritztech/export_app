...
if(isset($_POST['s'])) {
  ...
  die('Error: ' . mysql_error());
   ...
   echo "<script>alert('Data updated successfully.');location.href='a10creditnote_view.php'</script>"; 
...
try {
$query = "SELECT `cnid`, `supid`, `fid`, `siid`, `suppliername`, `billno`, `billvalue`, `claims`, `shortage`, `moisture`, `sand`, `oil`, `kirda`, `labour`,
 `cashdcond`, `brokerage`, `postage`, `bardanaclaims`, `bankcharges`, `other`,DATE_FORMAT(entrydate,'%d/%m/%Y')  as entrydate, `freight`, `gatepass`, 
 `rated`, `bankc`,tclaim,tpaid,debit,ledgername_n FROM `mandia10` WHERE cnid=?";
$stmt = $dbc->prepare($query);
$stmt->bindParam(1, $_GET['cnid']);
$stmt->execute();
...
function hledger1(t_actual,t_value) {
 ...
 var content = t_actual.options[idx].innerHTML;
  ...
  document.form1.debit.value = content;
...
<div align="center"><p align="center">&nbsp;</p>
  <h2 align="center"><span style="color:#F17327;">credit Note edit form</span></h2>
  ...
  <form id="form1" name="form1" onSubmit="return ValidateForm()"  method="post" action="">
    <table  border="1" rules="none" frame="box" cellpadding="6">
      <tbody>
        <tr>
          <td colspan="4"  bgcolor="#14C4B6"><h4>Credit Note Details</h4></td>
        </tr>
        <tr>
          ...
          <td width="200"><select name="supid" style="width:156px" onchange="hledger(this,this.value)">
            <option> </option>
            <?php               
        ...
        <tr>
          <td>Debit Ledger: </td>
          <td colspan="2"><input type="text" name="debit" size="40"  value="<?php echo $debit; ?>"/></td>
          <td><select name="abc" style="width:170px" onchange="hledger1(this,this.value)">
                       <option> </option>
                       <?php               
        ...
        <tr>
          ...
          <td><p>
            <input type="text" name="labour" id="textfield6" onkeyup = "claimtotal()"  value="<?php echo $labour; ?>" />
          </p></td>
        ...
        <tr>
          ...
          <td>Entry Date:</td>
          <td><input type="text" name="entrydate" onchange = "isDate(this.value)"  id="dstart" size="17" value="<?php echo $entrydate; ?>" />
        ...
        <tr>
          <td>&nbsp;</td>
          <td><input type="hidden" name="cnid" id="textfield13" value="<?php echo $cnid; ?>" />&nbsp;</td>
          <td><input type="submit" name="s"  value="Submit Record" /></td>
          <td>&nbsp;</td>
        </tr>
    </table>
  </form>
