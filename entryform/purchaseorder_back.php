	...
	if(isset($_POST['s']))
		{
            ...
            $brokername = trim(strip_tags(addslashes($_POST['brkname'])));
			...
			//$suppid = trim(strip_tags(addslashes($_POST['suppid'])));
			$totbag =  $_POST['totbag'];
...
$qry="INSERT INTO purchase_order (suppid,brokername,moisture,dust,smallseed,splitseed,oil,quantity,bag,packingterms,bagquality,rate,deleveryduedate,paymentterms,cashdcond,modeofsupply,saudadate,
stfcondition,mtfcondition,etfcondition,fid,entrydate,status,suppliername,bag1,
...
,STR_TO_DATE('$saudadate','%d/%m/%Y'),'$stfcondition','$mtfcondition','$etfcondition','$fid',STR_TO_DATE('$entrydate','%d/%m/%Y'),
'OPEN','$p_name','$totbag','$totgrsweight','$totmandi','$totbilwght','$totrot','$totrog','$totvalue','$totvto','$totbillv',$brkid)";
...
//header("Location: purchaseorder_front.php");
 ...
 $result1 = mysql_query("SELECT max(poid) from purchase_order");
...
VALUES ($row1[0],'$item','{$_POST['bagg'.$i]}','{$_POST['grsweight'.$i]}','{$_POST['mandiwght'.$i]}','{$_POST['billwght'.$i]}',
		...
		'{$_POST['billvalue'.$i]}','{$_POST['remark'.$i]}','A1',$stid1,'$p_name',STR_TO_DATE('$saudadate','%d/%m/%Y'),$fid,'{$_POST['w_per_bag'.$i]}')";
  ...
  {
	  echo "<script>alert('Data Successfully Inserted');location.href='purchaseorder_front.php'</script>";
