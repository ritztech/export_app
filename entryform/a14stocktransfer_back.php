...
$qry="INSERT INTO mandi14 (stockdate,stockitem,outqty,outvalue,fid,aceevalue,accdesc,stkid) VALUES 
(STR_TO_DATE('$stockdate','%d/%m/%Y'),'$stockitem','$outqty','$outvalue','$fid',$shortvalue,'$shortaccess','$stkid')";
...
for($i=1; $i <= $j; $i++)
 ...
 $qry1="INSERT INTO mandia14(formid,tempid,stockitem,inputqty,inputvalue,stkid,fid,tdate)VALUES
 ($row1[0],'A14','{$_POST['stkname'.$i]}','{$_POST['bagg'.$i]}','{$_POST['grsweight'.$i]}','{$_POST['item'.$i]}',$fid,STR_TO_DATE('$stockdate','%d/%m/%Y'))";
	//echo $qry1;
