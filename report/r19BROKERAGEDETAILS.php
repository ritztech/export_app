...
<div align="center">
  <h2 align="center"><span style="color:#F17327;">BROKERAGE RATE REGISTER REPORT </span>
    <script language="javascript" type="text/javascript" src="../datetimepicker.js"> </script>
</h2>
...
$result14=mysql_query("SELECT transactiondate,sum(bag),sum(netweight),sum(value2) FROM mandia4 WHERE transactiondate
 between STR_TO_DATE('$st_1','%d/%m/%Y') and STR_TO_DATE('$end_1','%d/%m/%Y') and fid=$fid  group by transactiondate");
...
{
$result13 = mysql_query("select DATE_FORMAT(a1.transactiondate,'%d/%m/%Y'),a1.brokername,a1.bag,a1.netweight,a1.partyname,a1.billtydate,
a1.value2,a1.paymentterms,a2.saudadate,
a2.total FROM mandia4 a1,mandia7 a2 where a1.fid=a2.fid and a1.transactiondate = STR_TO_DATE('$row14[1]','%d/%m/%Y') and a1.fid=$fid");
...
while($row13 = mysql_fetch_array($result13))
 ...
 <tr>
   ...
   <td> <input type = "text" size="5" readonly name = "bankcharges<?=  $i ?>" value = "------" />  </td>
   <td> <input type = "text" size="5" readonly name = "total<?=  $i ?>" value = "<?= $row13['6'] ?>" />  </td>
   <td> <input type = "text" size="5" readonly name = "modeofpay<?=  $i ?>" value = "<?= $row13['7'] ?>" />  </td>
   <td> <input type = "text" size="5" readonly name = "remark<?=  $i ?>" value = "<?= $row13['8'] ?>" />  </td>
    <td> <input type = "text" size="5" readonly name = "total<?=  $i ?>" value = "----------" />  </td>
   <td> <input type = "text" size="5" readonly name = "modeofpay<?=  $i ?>" value = "<?= $row13['9'] ?>" />  </td>
   <td> <input type = "text" size="15" readonly name = "remark<?=  $i ?>" value = "<?= $row13['10'] ?>" />  </td>
</tr>
