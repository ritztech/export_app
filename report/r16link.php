...
<td><input id="dend"  name = "dend"   type = "text" onchange = "isDate(this.value)"  size="15"  value="12/12/2018" />  <a href="javascript:NewCal('dend','ddmmyyyy')"><img src="../img/cal.gif" width="16" height="16" border="0" alt="Pick a date"></a></td>
...
$result18 = mysql_query("SELECT date,billno,freight_receivable,freight_type,billvalue1,bag1,billweight FROM mandia6  WHERE date 
between STR_TO_DATE('$st_1','%d/%m/%Y') and STR_TO_DATE('$end_1','%d/%m/%Y')  and 
fid=$fid and supid = '$v_partyid'");
...
while($row2 = mysql_fetch_array($result18))
  ...
  $fr_t1 = $row2['freight_type'];
    $freight_receivable = $row2['freight_receivable'];
	$sale_amt_1 = $row2['billvalue1'];
	
	$bag1 = $row2['bag1'];
	$billweight = $row2['billweight'];
	
	if($fr_t1 == 1)
	{
	  $dr_fr = $sale_amt_1 + $freight_receivable;
	}
...
('$row2[0]','SALES','','$row2[1]','$bag1','$billweight','0','0','$dr_fr','$cr_fr',$bal,$tid)"; 
...
while($row13 = mysql_fetch_array($result13))
 ...
 <tr>
   ...
   <?php   
	     ...
	     if ($v_crdr == 'DR')
		 {
			$close_balance = ($row13['8'] -  $row13['9']) + $jk - $open_balane_new; 
