...
$result1 = mysql_query("SELECT  `leg_name`, `o_bal`,dr_cr,brokerage,brok_qty FROM `ledger_master`  WHERE  legid=$abc");
...
<th> <span class="style3">Brokerage</span> </th> 
...
$broke_11 =  $brokerqge*$row13['4'];
 ...
 <tr>
   ...
   <td><input type = "text" size="7" readonly name = "tgrossqty<?=  $i ?>" value = "<?= $row13['4'] ?>" />   </td>
    <td><input type = "text" size="7" readonly name = "brkega<?=  $i ?>" value = "<?= $brokerqge ?>" />   </td>
 
   <td><input type = "text" size="7" readonly name = "tdebit<?=  $i ?>" value = "<?= $row13['8'] ?>" />   </td>
   <td> <input type = "text" size="7" readonly name = "tcerdit<?=  $i ?>" value = "<?= $broke_11 ?>" />  </td>
   ...
   <?php   
	     ...
	     if ($v_crdr == 'DR')
		 {
			$close_balance = ($broke_11 -  $row13['8']) + $jk - $open_balane_new; 
