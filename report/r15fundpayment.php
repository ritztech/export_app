...
<div align="center">
  <h2 align="center"><span style="color:#F17327;"> FUND PAYMENT REGISTER REPORT </span>
    <script language="javascript" type="text/javascript" src="../datetimepicker.js"> </script>
</h2>
...
$st_1 = $_GET['dstart'];
$end_1 = $_GET['dend'];
$result14=mysql_query("SELECT DATE_FORMAT(saudadate,'%d/%m/%Y'), sum( amount ),sum(bankcharges),sum(total)
FROM mandia7 WHERE saudadate between STR_TO_DATE('$st_1','%d/%m/%Y')  and STR_TO_DATE('$end_1','%d/%m/%Y') and fid=$fid
GROUP BY billid, saudadate");
...
<th> <span class="style3">Supplier Name</span> </th> 
 <th> <span class="style3">Bank Name</span> </th> 
<th> <span class="style3">Branch</span> </th>  
<th> <span class="style3">Cheque No</span> </th>
  <th> <span class="style3">Amount </span></th> 
 <th> <span class="style3">Bank Charges</span> </th>  
...
$result13 = mysql_query("select a1.suppliername,DATE_FORMAT(a1.saudadate,'%d/%m/%Y'),a2.bankname, a2.formid,a2.branch,
a2.chequeno,a2.amount,a2.bankcharges,a2.total,
a2.modeofpay,a2.remark,a2.tempid FROM mandia7 a1,mandia7_ref a2 where a2.formid=a1.billid and 
a1.saudadate =  STR_TO_DATE('$row14[0]','%d/%m/%Y') and a2.tempid='A7' and a1.fid=$fid");
...
while($row13 = mysql_fetch_array($result13))
 ...
 <tr>
   ...
   <td><input type = "text" size="14" readonly name = "saudadate<?=  $i ?>" value = "<?= $row13['1'] ?>" />    </td>
   <td> <input type = "text" size="30" readonly name = "suppliername<?=  $i ?>" value = "<?= $row13['0'] ?>" />  </td>
   <td> <input type = "text" size="18" readonly name = "bankname<?=  $i ?>" value = "<?= $row13['2'] ?> "/>  </td>
   <td> <input type = "text" size="18" readonly name = "branch<?=  $i ?>" value = "<?= $row13['4'] ?>" />  </td>
   <td> <input type = "text" size="12" readonly name = "chequeno<?=  $i ?>" value = "<?= $row13['5'] ?>" />  </td>
   <td><font color="#663300">  <?= $row13['6']?>  </font></td>
   <td> <input type = "text" size="7" readonly name = "bankcharges<?=  $i ?>" value = "<?= $row13['7'] ?>" />  </td>
   <td> <input type = "text" size="7" readonly name = "total<?=  $i ?>" value = "<?= $row13['8'] ?>" />  </td>
   <td> <input type = "text" size="7" readonly name = "modeofpay<?=  $i ?>" value = "<?= $row13['9'] ?>" />  </td>
   <td> <input type = "text" size="7" readonly name = "remark<?=  $i ?>" value = "<?= $row13['10'] ?>" />  </td>
</tr>
