...
<div align="center">
  <h2 align="center"><span style="color:#F17327;"> MANDI AND NIRASRIT TAX REPORT FOR ITEM PURCHASE </span>
    <script language="javascript" type="text/javascript" src="../datetimepicker.js"> </script>
</h2>
...
$st_1 = $_GET['dstart'];
$end_1 = $_GET['dend'];
$result14=mysql_query("SELECT DATE_FORMAT(recieptdate,'%d/%m/%Y'), sum( qtotal ),sum(amtotal),sum(ratetotal)
FROM mandia12m WHERE recieptdate between STR_TO_DATE('$st_1','%d/%m/%Y')  and STR_TO_DATE('$end_1','%d/%m/%Y') and fid=$fid
GROUP BY recieptdate");
...
<th> <span class="style3">Stock Item</span> </th> 
 <th> <span class="style3">Stock Quantity</span> </th> 
...
$result13 = mysql_query("SELECT DATE_FORMAT(a1.recieptdate,'%d/%m/%Y'),stockitem,quantity,rate,amount,remark FROM mandia12m a1,mandia12_ref a2,mandia12n a3
WHERE a1.mtaxid=a2.formidm and a3.ntaxid=a2.formidn and 
a1.recieptdate =  STR_TO_DATE('$row14[0]','%d/%m/%Y') and a1.fid=$fid");
...
while($row13 = mysql_fetch_array($result13))
 ...
 <tr>
   ...
   <td><input type = "text" size="14" readonly name = "saudadate<?=  $i ?>" value = "<?= $row13['0'] ?>" />    </td>
   <td> <input type = "text" size="30" readonly name = "suppliername<?=  $i ?>" value = "<?= $row13['1'] ?>" />  </td>
   <td> <input type = "text" size="18" readonly name = "bankname<?=  $i ?>" value = "<?= $row13['2'] ?> "/>  </td>
   <td> <input type = "text" size="18" readonly name = "branch<?=  $i ?>" value = "<?= $row13['3'] ?>" />  </td>
   <td> <input type = "text" size="12" readonly name = "chequeno<?=  $i ?>" value = "<?= $row13['4'] ?>" />  </td>
