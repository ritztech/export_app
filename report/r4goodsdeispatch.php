...
<div id="col4">
  <h2 align="center"><span style="color:#F17327;">GOODS DISPACTH REPORT - PURCHASE/ ORDER /BILLING</span>
    <script language="javascript" type="text/javascript" src="../datetimepicker.js"> </script>
</h2>
...
$result14=mysql_query("
 SELECT suppliername,DATE_FORMAT(dateofbillty,'%d/%m/%Y') as date , 
sum( bag1 ),sum(grossweight2),sum(mandigatepass1),sum(billweight),
sum(rateoftax1),sum(rateofgoods1),sum(value1), sum(vattax1),
sum(billvalue1)
 FROM goodrecieptsnote WHERE dateofbillty between STR_TO_DATE('$st_1','%d/%m/%Y') 
 and STR_TO_DATE('$end_1','%d/%m/%Y') and fid=$fid group by dateofbillty,suppliername");
...
{
$result13 = mysql_query("select a1.suppliername,DATE_FORMAT(a1.dateofbillty,'%d/%m/%Y'),
a2.stockid,a2.formid,a2.bag, a2.grswght, a2.mandiwght,a2.billwght,a2.rot,a2.rog,
a2.vale,a2.vtou,a2.bilvalue,a2.remark, a2.tempid FROM goodrecieptsnote a1,stock_ref a2 where a2.formid=a1.grid and 
dateofbillty= STR_TO_DATE('$row14[1]','%d/%m/%Y') and a1.suppliername='$row14[0]' and a1.fid=$fid and a2.tempid='A2'");
